...
use \ElementsKit\Elementskit_Widget_Gallery_Handler as Handler;
...
class Elementskit_Widget_Gallery extends Widget_Base
{
    ...
    public function __construct( $data = [], $args = null ) {
		...
		$this->add_script_depends('masonry');
        $this->add_script_depends('tilt');
	}
    ...
    protected function render_raw( ) {
        ...
        extract($settings);
            $col = [
                    'desktop'=>$ekit_gallery_columns,
                    'tablet'=>$ekit_gallery_columns_tablet,
                    'mobile'=>$ekit_gallery_columns_mobile
            ];

            $this->add_render_attribute( 'gallerycol', 'data-gallerycol', wp_json_encode($col) );
    ?>
        <?php
            $gallery_grid_class = '';
        ...
        ?>
            <div class="elemetskit_filter_wraper_outer <?php if($ekit_gallery_fill_nav_content != 'yes') : ?> elemetskit_filter_<?php echo esc_attr($ekit_gallery_nav_alignment); ?> <?php endif; ?>">
                <div class="filter-button-wraper" id="ekit_gallery__filter<?php echo esc_attr($this->get_id());?>">
                    <ul class="option-set elementskit-main-filter elementskit_filter_<?php echo esc_attr($ekit_gallery_filter_style_choose); ?> <?php if($ekit_gallery_nav_item_show_caret == 'yes'){echo esc_attr('elementskit_nav_caret'); } ?> <?php if($ekit_gallery_filter_show_divider == 'yes') {echo esc_attr('elementskit_divider_active');}?>" data-option-key="filter">
        ...
        <?php endif; ?>
        <?php echo '<div class="ekit_gallery_grid_wraper"><div id="ekit_gallery_'.$this->get_id().'" class="ekit_gallery_grid" '.$this->get_render_attribute_string( 'gallerycol' ).'>'; ?>
        <?php
        ...
        <?php if(count($ekit_gallery_items) > 0): foreach($ekit_gallery_items as $item): // ekit_gallery_items // ekit_gallery_filter_images
            ...
            foreach( $item['ekit_gallery_filter_images'] as $img ):
                $metadata = \ElementsKit\Utils::img_meta($img['id']);
                ?>
                <?php if(isset($ekit_gallery_enable_link) && $ekit_gallery_enable_link === 'yes') :

                    $web_link = (! empty( $ekit_gallery_web_link['url'])) ? $ekit_gallery_web_link['url'] : '';
                    $web_link_target = ( $ekit_gallery_web_link['is_external']) ? '_blank' : '_self';
                    $web_link_nofollow = ( $ekit_gallery_web_link['nofollow']) ? 'nofollow' : '';
                ?>
                    <a href="<?php echo esc_url($web_link); ?>" target="<?php echo esc_attr($web_link_target); ?>" rel="<?php echo esc_attr($web_link_nofollow); ?>">
                <?php endif; ?>
                <div class="ekit_gallery_grid_item <?php echo esc_attr($this->clean_class($item['ekit_gallery_filter_label']).' '.$gallery_grid_class); ?>">
                    ...
                    <?php if($ekit_gallery_tilt_show_glare == 'yes'): ?>
                    data-tilt-glare="<?php echo esc_attr($ekit_gallery_tilt_show_glare); ?>"
                    data-tilt-maxglare="<?php echo esc_attr($ekit_gallery_tilt_maxGlare['size']); ?>"
                    <?php endif; ?>
                    ...
                    data-tilt-axis="<?php echo esc_attr($ekit_gallery_tilt_axis); ?>"
                    <?php endif; ?>
                    >
                        ...
                        <?php endif; ?>
                        >
                        <img class="elementskit-grid__img" src="<?php echo esc_url( $img[ 'url' ] ); ?>" alt="<?php echo $metadata['alt'] == '' ? 'gallery grid image' : esc_attr($metadata['alt']); ?>"/>
                        </div>
                            ...
                            <div class="elementskit-hover-content ekit_vertical_alignment_<?php echo esc_attr($ekit_gallery_content_vertical_alignment); ?>">
                                <?php
                                    if ($ekit_gallery_caption == 'yes') {
                                        echo \ElementsKit\Utils::kses($metadata['caption'] != '' ? '<h3 class="elementskit-gallery-title">'.$metadata['caption'].'</h3>' : '');
                                    }
                                ?>
                                <?php
                                    if ($ekit_gallery_description == 'yes') {
                                        echo \ElementsKit\Utils::kses($metadata['description'] != '' ? '<p class="elementskit-gallery-description">'.$metadata['description'].'</p>' : '');
                                    }
                                ?>
                                <?php
                                    if ($ekit_gallery_filter_label_show == 'yes' ) {
                                       
                                        echo \ElementsKit\Utils::kses($item['ekit_gallery_filter_label'] != '' ? '<span class="elementskit-gallery-label">'.$item['ekit_gallery_filter_label'].'</span>' : '');
                                    }
                                ?>
    ...
    protected function render_editor_script() {
        ...
        $ekit_gallery_columns = $settings['ekit_gallery_columns'];
        $ekit_gallery_columns_tablet = $settings['ekit_gallery_columns_tablet'];
        $ekit_gallery_columns_mobile = $settings['ekit_gallery_columns_mobile'];
         ?>
        <script type="text/javascript">
            jQuery( document ).ready( function( $ ) {
                $('#ekit_gallery_<?php echo esc_attr($this->get_id()); ?>').each( function() {
                    var $container = $(this);
                    if ($container.length > 0) {
                        ...
                        var w = $container.width(),
                            ...
                            columnWidth = 0;
                        if (w > 1024) {
                            columnNum = <?php echo esc_attr($ekit_gallery_columns) ?>
                        } else if (w > 600) {
                            columnNum = <?php echo esc_attr($ekit_gallery_columns_tablet) ?>
                        } else if (w > 480) {
                            columnNum = <?php echo esc_attr($ekit_gallery_columns_mobile) ?>
                        }
                    ...
                    var $optionSets = $('#ekit_gallery__filter<?php echo esc_attr($this->get_id())?>.filter-button-wraper .option-set'),
