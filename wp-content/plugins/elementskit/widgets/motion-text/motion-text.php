...
use \ElementsKit\Elementskit_Widget_Motion_Text_Handler as Handler;
...
class Elementskit_Widget_Motion_Text extends Widget_Base {
    ...
    protected function render_raw( ) {
		...
		$target = $settings['ekit_motion_text_sub_title_website_link']['is_external'] ? '_blank' : '_self';
		$nofollow = $settings['ekit_motion_text_sub_title_website_link']['nofollow'] ? 'nofollow' : '';
		// motion main class
		$motion_main_class = '';
		// anim class
		$anim_motion_class = '';
		...
		<div class="elementskit_motion_text_wraper">
			...
			<?php
				echo '<' .$settings['ekit_motion_text_sub_title_tag']. ' class="ekit_motion_text_title '.$motion_main_class.'  '.$ekit_text_spilit_class.' '.$ekit_text_spilit_one_by_one_class.'" data-ekit-animation-delay-s="10" data-animate-class="'. $anim_motion_class .'">';
				if ($url !== '') {
					echo '<a href='.$url.' target="'.esc_attr($target).'" rel="'.esc_attr($nofollow).'"><span class="ekit_motion_text" data-ekit-animation-delay="'.esc_attr($animation_delay_char_based).'">'.\ElementsKit\Utils::kses( $title_text ).'</span></a>';
				} else {
					echo '<span class="ekit_motion_text" data-ekit-animation-delay="'.esc_attr($animation_delay_char_based).'">'.\ElementsKit\Utils::kses( $title_text ).'</span>';
				} ?>
				<?php echo '</'.$settings['ekit_motion_text_sub_title_tag'].'>';
			?>
