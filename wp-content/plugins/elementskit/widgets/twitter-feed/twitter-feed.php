...
use \ElementsKit\Elementskit_Widget_Twitter_Feed_Handler as Handler;
...
class Elementskit_Widget_Twitter_Feed extends Widget_Base {
        ...
        if(strlen($textData) > 5){
            $explodeText = explode(' ', trim($textData));
             for($st = 0 ; $st < count($explodeText) ; $st++){
                ...
                if($pos !== false){
                    $stringText .= '<a href="https://twitter.com/hashtag/'.str_replace('#', '', $explodeText[$st]).'?src=hash" target="_blank"> '.$explodeText[$st].' </a>';
                }else if($pos1 !== false){
                    $stringText .= '<a href="https://twitter.com/'.$explodeText[$st].'/" target="_blank"> '. $explodeText[$st].' </a>';
    ...
    protected function render_raw( ) {
		   ...
		   $styleLayout = $ekit_twitter_feed_layout_style; echo '</pre>';
		   ...
		   <?php
				...
				?>
					<p class="ekit-twitter-error"> <?php esc_html_e( isset($timeline['errors'][0]['message']) ? $timeline['errors'][0]['message'] : 'Invalid author'); ?> </p>
				<?php
					return ;
				}
				if( isset($timeline['error']) ){
				?>
					<p class="ekit-twitter-error"> <?php esc_html_e( isset($timeline['error']) ? $timeline['error'] : 'Invalid author'); ?> </p>
				<?php
					return ;
				}
			?>
							...
							?>
								<div class="ekit-twitter-feed-column <?php echo esc_attr($columnFeed); ?>">
									<div class="ekit-twitter-feed-content-wraper">
												...
												<div class="ekit-twitter-feed-posts-item-user">
													...
													<div class="profile-display-name">
														...
														<div class="ekit-twitter-feed-item-user-screen-name">
															<a href="<?php echo 'https://twitter.com/'.$user['screen_name']  ;?>" class="screen_name">
															...
															<span class="ekit-twitter-feed-item-user-date">
																<datetime><?php echo esc_html(date(get_option('date_format'), strtotime($twitter['created_at']) ));?></datetime>
															</span>
															<?php endif;?>
															<!--end date -->
														</div>
										...
										<p class="feed-title"><?php echo \ElementsKit\Utils::render($this->str_check($twitter['text']));?> </p>
										<?php
