...
use \ElementsKit\ElementsKit_Widget_Woo_Product_List_Handler as Handler;
...
class Elementskit_Widget_Woo_Product_List extends Widget_Base {
		...
		<div class="ekit-wc-product-image ekit-background-cover">
			...
			?>
				<div class="ekit-wc-add-to-cart ekit-cart-align-<?php echo esc_attr($settings['ekit_button_alignment']) ?> ekit-cart-vertical-align-<?php echo esc_attr($settings['ekit_button_vertical_alignment']) ?>">
					<?php
						$add_cart_btn = '<a data-product_id="'. $product->get_id() .'" href="'. $product->add_to_cart_url() .'" class="'.$iconPos . ' ' . $iconCls . ' button product_type_simple add_to_cart_button ajax_add_to_cart">'. $product->add_to_cart_text() .'</a>';
	...
	public function render_description() {
		$settings = $this->get_settings();
		global $product;
		// var_dump($product->get_rating_count());
		?>
							...
							<div class="ekit-wc-rating">
								...
								<?php 
									if($product->get_rating_count() > 0){
										...
										echo \ElementsKit\Utils::render($rating_html);
									}
								?>
							</div>
						<?php endif; ?>
	...
	public function render_loop_item() {
		...
		if($wp_query->have_posts()) {
			...
			$this->add_render_attribute('ekit-wc-products-wrapper', 'ekit-grid', '');
