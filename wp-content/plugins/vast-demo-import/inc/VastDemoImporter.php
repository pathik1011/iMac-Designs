...
class VastDemoImporter {
	...
	/**
	 * The instance of the VAST_OCDI\Importer class.
	 *
	 * @var object
	 */
	public $importer;
	 ...
	 *
	 * @return VastDemoImporter the *Singleton* instance.
	 */
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_VAST_OCDI_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
		add_action( 'wp_ajax_VAST_OCDI_import_customizer_data', array( $this, 'import_customizer_data_ajax_callback' ) );
		add_action( 'wp_ajax_VAST_OCDI_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters(
			'vt-VAST_OCDI/plugin_page_setup', array(
				'parent_slug' => 'themes.php',
				'page_title'  => esc_html__( 'Vast Demo Import', 'vdi' ),
				'menu_title'  => esc_html__( 'Vast Demo Import', 'vdi' ),
				'capability'  => 'import',
				'menu_slug'   => 'vast-one-click-demo-import',
			)
		);
		...
		$this->plugin_page = add_submenu_page(
			...
			apply_filters( 'vt-VAST_OCDI/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);

		register_importer( $this->plugin_page_setup['menu_slug'], $this->plugin_page_setup['page_title'], $this->plugin_page_setup['menu_title'], apply_filters( 'vt-VAST_OCDI/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ) );
	}
	...
	public function display_plugin_page() {
		require_once PT_VVAST_OCDI_PATH . 'views/plugin-page.php';
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_enqueue_script( 'VAST_OCDI-main-js', PT_VVAST_OCDI_URL . 'assets/js/main.js', array( 'jquery', 'jquery-ui-dialog' ), PT_VVAST_OCDI_VERSION );

			// Get theme data.
			$theme = wp_get_theme();

			wp_localize_script(
				'VAST_OCDI-main-js', 'VAST_OCDI',
				array(
					'ajax_url'         => admin_url( 'admin-ajax.php' ),
					'ajax_nonce'       => wp_create_nonce( 'VAST_OCDI-ajax-verification' ),
					'import_files'     => $this->import_files,
					'wp_customize_on'  => apply_filters( 'vt-VAST_OCDI/enable_wp_customize_save_hooks', false ),
					'import_popup'     => apply_filters( 'vt-VAST_OCDI/enable_grid_layout_import_popup_confirmation', true ),
					'theme_screenshot' => $theme->get_screenshot(),
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'vdi' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'vdi' ),
						'dialog_no'             => esc_html__( 'Cancel', 'vdi' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'vdi' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'vdi' ),
					),
					'dialog_options'   => apply_filters( 'vt-VAST_OCDI/confirmation_dialog_options', array() ),
				)
			);

			wp_enqueue_style( 'VAST_OCDI-main-css', PT_VVAST_OCDI_URL . 'assets/css/main.css', array(), PT_VVAST_OCDI_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', apply_filters( 'vt-VAST_OCDI/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			 ...
			 * Manually uploaded import files or predefined import files via filter: vt-VAST_OCDI/import_files
			 */
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'vdi' );
			} elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: vt-VAST_OCDI/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'vdi' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'vdi' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'vdi' ) );
			}
		}

		// Save the initial import data as a transient, so other import parts (in new AJAX calls) can use that data.
		Helpers::set_VAST_OCDI_import_data_transient( $this->get_current_importer_data() );

		if ( ! $this->before_import_executed ) {
			$this->before_import_executed = true;

			/**
			 * 2). Execute the actions hooked to the 'vt-VAST_OCDI/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			do_action( 'vt-VAST_OCDI/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'vt-VAST_OCDI/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		do_action( 'vt-VAST_OCDI/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== has_action( 'vt-VAST_OCDI/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'vt-VAST_OCDI/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'vt-VAST_OCDI/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		delete_transient( 'VAST_OCDI_importer_data' );

		// Display final messages (success or error messages).
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = '';

			if ( ! apply_filters( 'vt-VAST_OCDI/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome! Thanks @ProteusThemes! #VAST_OCDI https://www.proteusthemes.com/', 'vdi' );

				$response['message'] .= sprintf(
					__( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Created and maintained by %3$sProteusThemes%4$s. %2$s%5$sClick to Tweet!%4$s%8$s', 'vdi' ),
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly..%5$s', 'vdi' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'vdi' ),
	...
	private function use_existing_importer_data() {
		if ( $data = get_transient( 'VAST_OCDI_importer_data' ) ) {
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'vdi', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( apply_filters( 'vt-VAST_OCDI/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'vt-VAST_OCDI/after_content_import_execution' action hook.
		 */
		...
		$importer_options = apply_filters(
			'vt-VAST_OCDI/importer_options', array(
		...
		$logger_options = apply_filters(
			'vt-VAST_OCDI/logger_options', array(
