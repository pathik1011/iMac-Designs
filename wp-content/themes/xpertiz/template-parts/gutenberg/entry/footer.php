...
<div class="entry__footer-wrapper">
  <div class="entry__more_link">
	...
	if ( ! is_single() && ! is_page() ) {
		if ( xpertiz_has_more_link( get_the_ID() ) ) {
			$xpertiz_more_link = '<a href="' . esc_url( get_permalink() ) . '" class="more-link">' . esc_html__( 'Read More', 'xpertiz' ) . '</a>';
			$xpertiz_more_link = apply_filters( 'the_content_more_link', $xpertiz_more_link );
			echo wp_kses( $xpertiz_more_link, 'post' );
		}
	}
	?>
  </div>
